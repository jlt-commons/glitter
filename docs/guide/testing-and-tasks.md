...
`jolt counter`, `jolt todo`, `jolt crud`, `jolt flights`,
`jolt temperature`, and `jolt timer` are the interactive examples; the
full quick-start demo from `docs/guide/index.md`, a larger task-board
demo (ported from glimmer's own `todo.clj`) exercising derived counts,
a value-bearing `:change` handler, and checkbutton toggles, a port of
...
derived sort order, and `:sensitive`-gated buttons, a port of the
[7GUIs Flight Booker task](https://eugenkiss.github.io/7guis/tasks/#flight-booker)
exercising constraints between and within widgets via `glitter.nexus`
(see [`nexus.md`](nexus.md)), a port of the [7GUIs Temperature
Converter task](https://eugenkiss.github.io/7guis/tasks/#temp)
exercising two linked `:entry` fields via a single `glitter.nexus`
action-expansion, and a port of the [7GUIs Timer
task](https://eugenkiss.github.io/7guis/tasks/#timer) exercising a
background `:effect/schedule` tick and a live `System/nanoTime` read
at render time (not through `:nexus/system->state`; see
[`nexus.md`](nexus.md)): meant to be run and clicked, not asserted
on. `jolt todo` and `jolt crud` are also `glitter.nexus` consumers as
of this arc (retrofitted from a hand-written `execute-actions` `case`
...
followed by `jolt temperature` and `jolt timer`.
...
**One adaptation beyond the b12n-raylib-jlt original**: `:pointer` return values

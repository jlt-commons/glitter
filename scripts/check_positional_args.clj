...
(def file-pattern
  "Glob pattern for files to check. NOT the adk-clj original's \"**/*.clj\" —
  verified live that babashka.fs/glob's \"**\" requires at least one
  directory level, so it silently matches ONLY files in subdirectories and
  misses every file sitting directly in source-dirs (glitter's src/glitter
  is flat — 17 direct .clj children, no subdirs — so the original pattern
  would find zero of them). \"{*,**/*}.clj\" matches both."
  "{*,**/*}.clj")
